...
#                   
#                 v5  after talking with he mentioned the data was not reliable without checking carefully the calls available. 
#                     I filter and those calls, added proper zero for when a site was monitored but no bats were found,
#                     
#
...
#
# Inputs:
#   - data_for_analysis/prep_for_glmm_v2/bm2.csv         : Bat call counts, predictors, standardized 
#   - data_for_analysis/prep_for_glmm_v2/bm.miller.day.csv : Activity index and trait data
#
# Outputs:
...
# Load the data
bm2 <- read_csv("data_for_analysis/prep_for_glmm_v2/bm2.csv") %>%
  clean_names()

glimpse(bm2) # check the structure of the data
...
# model with a negative binomial has a better c-hat value of 0.97 although still overdisperssed
# now we try adding seasonality as fixed effects. R conditional is around 0.14 and marginal 0.095

# chat gpt  to add random effects and slops first 
