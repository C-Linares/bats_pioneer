...
bat_combined<-read_csv(file = 'data_for_analysis/prep_for_glmm/bat_combined.csv') # 70 rows are missing the time for the date time column. see below. instead of using bat combined we will use bat_zero_db from the prep_for_glmm_v2 script. 

bat_zero_db # this one does not work because 


# filter noise out

bat_combined<- bat_zero_db %>% filter(sp !="Noise")
...
site.date<- bat_zero_db %>% select(site, date_time)
...
# code built with ai assitance --------------------------------------------

# this code helps recalcuate the moon illumination to assure there are no missing dates when mergin with the bat calls data set bm2

# create every monitored site-night from bat_zero_db

sidalo<- bat_zero_db %>% 
  distinct(site, noche) %>%
  left_join(
    sts %>% select(site, lat, lon),
    by = "site"
  ) %>% 
  mutate(
    date_time= as.POSIXct(
      paste(noche, "12:00:00"),
      tz= "America/Denver")
  )
...
moon.stat.list <- parLapply(
  cl,
  seq_len(nrow(sidalo)),
  function(i, moon_input) {
...
moon.df <- bind_rows(moon.stat.list)

moon.stat.complete <- bind_cols(
  sidalo %>%
    select(site, noche, lat, lon),
  moon.df
)


# create regular time points 
moon_time_grid <- moon.stat.complete %>%
  select(site, noche, lat, lon, sunset, sunrise) %>%
  rowwise() %>%
  mutate(
    date_time = list(
      seq(
...
moon_time_grid %>%
  select(site, noche, date_time, lat, lon) %>%
  arrange(site, noche, date_time)


# calculate moon at every timestamp

print("Calculating moonlight intensity...")

system.time(
  moon.int.raw <- calculateMoonlightIntensity(
    moon_time_grid$lat,
    moon_time_grid$lon,
    moon_time_grid$date_time,
    e = 0.16
  )
)
...
# keep night time only 
moon.int.night <- moon.int %>%
  filter(night == TRUE)

# calculate one row per site-night 

moon_daily_avg_complete <- moon.int.night %>%
  group_by(site, noche) %>%
  summarise(
    avg_moonlight = mean(
      moonlightModel,
...
missing_after_recalculation <- bat_zero_db %>%
  distinct(site, noche) %>%
  anti_join(
    moon_daily_avg_complete %>%
      distinct(site, noche),
    by = c("site", "noche")
  )

missing_after_recalculation # zero missing night so now it is ready to merge with the data. 
...
moon_daily_avg_complete # this is the file that should be exported and is going to replace moon.int.night 
glimpse(moon_daily_avg_complete)
...
fwrite(moon_daily_avg_complete, file = "data_for_analysis/moon_pred_v2/moon_daily_avg_complete.csv", row.names = FALSE)
# 

# Create a README  to explain the moon.stat and moon.int data sets.
readme_content <- " Moonlight predictor files

Carlos Linares  
Originally created: February 11, 2025  
Updated: July 16, 2026  

This directory contains moonlight predictors generated with the `moonlit` R package, in collaboration with Kyle Shannon. Moonlight values were calculated for every monitored site-night, including nights when no bats were detected.

## Files

### `sidalo.csv`

Contains one row for each monitored site-night and provides the input information required by `moonlit`.

Variables include:

- `site`: study-site identifier.
- `noche`: date assigned to the monitoring night.
- `date_time`: local date-time used to identify the night.
- `lat`: site latitude.
- `lon`: site longitude.

### `moon.stat.csv`

Contains nightly moonlight statistics returned by `calculateMoonlightStatistics()`. In the R script, these results are stored in the `moon.df` object before being exported.

Each row represents one monitored site-night.

Variables include:

- `sunset`: local time of sunset.
- `sunrise`: local time of sunrise.
- `meanMoonlightIntensity`: mean modeled moonlight intensity during the night.
- `minMoonlightIntensity`: minimum modeled moonlight intensity during the night.
- `maxMoonlightIntensity`: maximum modeled moonlight intensity during the night.
- `meanMoonPhase`: mean illuminated fraction of the Moon during the night.
- `minMoonPhase`: minimum illuminated fraction of the Moon during the night.
- `maxMoonPhase`: maximum illuminated fraction of the Moon during the night.

### `moon.int.csv`

Contains modeled illumination at regular one-hour intervals from sunset to sunrise for every monitored site-night.

Variables include:

- `site`: study-site identifier.
- `noche`: date assigned to the monitoring night.
- `date`: date and time of the modeled observation.
- `lat`: site latitude.
- `lon`: site longitude.
- `night`: logical value indicating whether the Sun was below the horizon.
- `sunAltDegrees`: solar altitude in degrees.
- `moonlightModel`: predicted moonlight intensity relative to an average full moon.
- `twilightModel`: predicted twilight illumination in lux.
- `illumination`: combined modeled moonlight and twilight illumination in lux.
- `moonPhase`: illuminated fraction of the Moon.

### `moon.int.night.csv`

Contains the same variables as `moon.int.csv`, but retains only observations for which `night == TRUE`, meaning that the Sun was below the horizon.

### `moon_daily_avg_complete.csv`

Contains nightly illumination averages calculated from `moon.int.night.csv`. Each row represents one monitored site-night.

Variables include:

- `site`: study-site identifier.
- `noche`: date assigned to the monitoring night.
- `avg_moonlight`: mean nightly value of `moonlightModel`, expressed relative to an average full moon.
- `avg_twilight`: mean nightly value of `twilightModel`, expressed in lux.
- `avg_illumination`: mean nightly combined moonlight and twilight illumination, expressed in lux.
- `avg_moon_phase`: mean illuminated fraction of the Moon during the night.
- `n_timepoints`: number of modeled time points used to calculate the nightly averages.

## Important note

Earlier versions of these predictors were generated from the date-times of detected bat calls. The updated workflow instead uses the complete site-night monitoring grid from `bat_zero_db`. This ensures that moonlight predictors are available for monitored nights with zero bat detections and that environmental predictors are calculated independently of bat activity.
"
...
writeLines(readme_content, "data_for_analysis/moon_pred_v2/README.txt")
...
save.image(file = "working_env/moon_pred_v2.RData")
