...
I am including light as lux and watts in a model to see how it performs.

apparently we have several weeks in 2023 where there is no insect data 5659 lines. I figure I have to check those NAs and make them into the mean of that site for that years season. that way we will have the data correctly.

the light data has some errors, long 02 has no data for one of the years.

------------------------------------------------------------------------

7/31/2025

- I was looking at the moon data and it seems we are missing data for the early period of sampling 21-22 06 2021 and for 2023-09-05

  |            |     |
  |------------|-----|
  | 2023-09-05 |     |

  I might need to recheck the moon script to produce the estimates.

- updated the glmm_v2 and found included light data from the spectral measurements. Now I am testing model m1.6nb to see if it does a better job.

------------------------------------------------------------------------

8/5/2025

It seems we need to fix the moon prediction scrip to be able to get the 110 na rows we are missing.

- I have to check how many calls we actually have. It seems we might be missing some calls. the bat combine data has the following

  ![](images/clipboard-2755547527.png){width="781"}

  but the bm2 file that I am using for modeling has a lot less. WHY? in the paper I am reporting 17 000 ish calls but M.lucifugus has 241 thousand by itself. Something does not match.

- R has been crashing and I think I know why now. ti seems like one of the packages might be giving trouble.

------------------------------------------------------------------------

8/6/2025

R keeps crashing and I suspect is the glmm_v2 script. I will start a new glmm_v3 script and we will review the bat call data base that might have a few calls missing. I will recalculate the moon intensity because it seems it's missing some dates.

- when mergin the moon variable with the bat calls using date, I will make sure to have them both in the same time zone. that might be making things fail.

- I also need to check the bat bm.csv data file. I am suspecting we are missing a bunch of calls for each sepecies. Solved : the error came from reporting the total number of lines for each species instead of the total number of counts for each species. we are not missing calls.

- Update: I think I found the problem with the moon and the bat data. When I calculate the noche column a date disapear becuase it gets converted all to the same date, but there's still calls on that other date. I think the solution might be to calculate the moon intensity with a buffer around the dates of bat calls.\

  ------------------------------------------------------------------------

8/07/2025

I will recalculate the moon variable but using one date around the min and max date of recordings for bats. That way we will have enough data.\
\
- I think I realized that if there is a unproperly done merge R crashes.\

------------------------------------------------------------------------

08/08/2025

I started preparing for working on the overlap analysis for bats. to see if their activity patters change between sites. I created the bat_delta_sunset.R

------------------------------------------------------------------------

8/28/2025

I managed to figure out how to run a mvGLM and dbrd now I am trying to run rank abundance curves.

------------------------------------------------------------------------

3/18/2025

I am building the data base for the robomoth and speakes to analyse the data.

there are some files for 2021 and 22 that seems like they need to be ran through sonobat.

```{r}
files_to_check
```

I also have to remove noise tags.

I need to clean by time stamp too. any even detected after 8 am should be taken out too.

------------------------------------------------------------------------

3/21/2024\
\
still working on the missing vetted files. ======= 1/23/2025

I revisit the effort calculation in the prepr_for_glmm.R for days and hours. It seems like I can't reliably calculate effort by day in hours because I am getting some zeros. I need to correct this issue but I am unsure on how to achieve this. to calculate it I need to take in consideration all the recorded files from the very first to the very last by site and day. I was just using the correctly ID ones and that is wrong. The idea was to calculate a nightly call rate. I think I will use the total bat calls

------------------------------------------------------------------------

3/23/2026

I had an issue with github and was giving me trouble after I did some work on desktop computer. I lost the notes I did in the desktop computer. I should be careful now about pulling and pushing the notes file.\
\
I will start working on the robomoth_speakr_models.R script to analyze the data for the robomoth and speakers.

-issue: I discovered an issue in the speaker data. the site vizc04 has a bunch of NAs probably because it has a misspelled tag vizcc04 instead of vizc 04

I will fix this but this data will have to wait.

I created the rob_db.csv, this is ready for analysis with predictors standardized. However, the spkr data still needs fix.

------------------------------------------------------------------------

3/24/2026

I have been running models for the robomoth data, but I realized that I need to change one of the predictors. The temp and wind are correlated (0.73) so I will try one.\
![](images/clipboard-3012166694.png)

one paper Li et al 2024 indicates that both wind and temperature affect bat activity like us they found these are correlated. I think we should keep just wind as it might mess with predation more than temperature.\
\
I produced several marginal effects plots for the robomoth data.

------------------------------------------------------------------------

3/25/2026

I updated the insect predictor from c_bugs to ins_bm, and I have reworked the speakr_all data as it was missing some manual vetting.\
\
I am updating the models for the robomoth data today. I also updated the treatment so it is dark= -1 and lit = 1

------------------------------------------------------------------------

3/26/2026

I did some marginal effects plots for the bat paper.\
i removed temp to see if the model m11 changed. I should readd it later

------------------------------------------------------------------------

3/28/2026

keept working on the marginal effects plots

------------------------------------------------------------------------

3/29/2026

I completed the initial set of plots for the spkr model.

created script glmm_v4.R to reanalyze the bat actrivity data. Arrived to model m1.8 as the best model.

------------------------------------------------------------------------

3/30/2026

I built the marginal effects plot for the final model (m1.8) for bat echolocations across experimental sites.

------------------------------------------------------------------------

5/6/2026

After talking with Jesse there's some bat species that we need to filter or manually vet their calls. Some species are easily confused with others when analyzed acoustically, thus we need to change the set some rules for those calls that need to be vetted manually and for those that might change to mytis sp or just hifreq calls depending on the case.\

------------------------------------------------------------------------

5/7/2026\
\
I created a script to update the bat matrix that we use in the glm_v4 script. The prep_for_glmm_v2 now will produce te new matrix to be analyzed on a new glm script later.

------------------------------------------------------------------------

5/8/2026

I am building the rules for cleaning the bat data and reanalize it later.\
I am still working on the rules to analyze the data.

------------------------------------------------------------------------

5/16/2026

I realized that for the robomoth data there are some files duplicated for 2022 data\
...
# A tibble: 708 × 2
   filename_norm                     n
   <chr>                         <int>
 1 iron03-20220713_222340[1].wav     2
 2 iron03-20220713_222355[1].wav     2
 3 iron03-20220713_222410[1].wav     2
 4 iron03-20220713_222425[1].wav     2
 5 iron03-20220713_222440[1].wav     2
 6 iron03-20220713_222455[1].wav     2
 7 iron03-20220713_222510[1].wav     2
 8 iron03-20220713_222525[1].wav     2
 9 iron03-20220713_222540[1].wav     2
10 iron03-20220713_222555[1].wav     2
# ℹ 698 more rows
...
\
\
I discovered that iron03 has a second file with the same dates for

|                               |     |
|-------------------------------|-----|
| iron03-20220713_222340[1].wav |     |

I moved the duplicated files to a trash folder inside the iron03 folder.

I will remove the folder called "second to a file that does not need to be analyze.\
Iron 05 also have duplicated files. needs to be rerun with the sonobat buzz and with the amp script

long03 also had double files product of the split larger files.

long05

vizc04

all of this for 2022

------------------------------------------------------------------------

5/18/2026

I removed the files that seemed duplicated, now I should have files that are ready to go. 2023 seemd not to have the same problem. I think I need to recreate the robomoth outputs from sonobat first. I discovered how to add the kpro into the kpro autoid column and that could help me make more reproducible content.

------------------------------------------------------------------------

5/31/2026

I generated a graph of bat calls and the amp frequency. I need to ask what will be the process to select low frequency calls. I think we have to select certain bats to achieve this.\
I don't know how this works yet. I also need to keep working on the code todo the same for speaker data.\
\
![](images/clipboard-2015532310.png)

------------------------------------------------------------------------

6/01/2026

I re coded the calls for myyo, myca, and myth as mysp and euma as low frequecy.\
I need to do the same for the speaker data and merge it with the amp data.\
I also need to find a cut off value to keep faint calls.\
\
...
Improved on the graph to use it as supplements and discuss with Jesse.\
![](images/clipboard-802186843.png)

------------------------------------------------------------------------

6/05/2026

I ran into a problem with the amp files for the spkr data. There are several files that are duplicate because there is duplicate data for some sites in 2023. I will come back to this after I have remove those files and reran the amplitude script on that folder.

step 1 : remove unecessary folder.

step 2: reproduce the buzz vetting table

step 3: reproduce the amplitude data.\
\
for the sm3 data and the buzz counts, I have encounter a great deal or trouble as the ID data seems to be missing and other files are taking long to run. I have decided to focus ont he buzzes later on R merge the buzz data witht he consolidated observations I already have.

------------------------------------------------------------------------

6/06/2026

I have removed the unnecessary folders and reproduce the buzz vetting tables for sites iron 4 and 5 on 2023 speaker data.\
\
reviewing the amp files it seems like we have another problem. We have data for the iron 05 site doubled. we have the same dates twice meaning there's a mistake or a recorder was labelled incorrectly. I think I am inclining to remove this site from the analysis.\
an alternative is to remove add an additional column as identifier to the amp data to be able to merge them without problem. an extra alternative is to just chose one set of data and rerun the amp and reexport the buzz.

------------------------------------------------------------------------

6/15/2026

I think I am going to opt for keeping iron05 2023 out for the moment.\
\
observation: it seems like there are more bat passes with higher amplitude at lit sites than dark sites. I wonder if this is something to consider. It could be they are just calling closer to the mics at lit site because they are close to the lights and tripods where the microphones are deployed.

I discover some issues with the sonobat out put tables where the files were not properly aligned. I had 2023 tables in 2022 folders.\
I am looking at path\
\
F:\pioneer\_2022\bat\_speakers\speaker\_organized\
...
those sites need sonobat output tables rexported. I fixed this issue.\
\
now we have new files for the speaker and robomoth data. We can now test the idea that there might be less faint calls/passes at lit sites vs dark sites.

------------------------------------------------------------------------

6/17/2026

the objective right now is to get the robomoth and speaker files loaded so I can start analyzing them after I filter out the ones with amp more that -20 dbfs

- load files\
  load predictors

- filter out loud calls

I will use the robo_spkr_prep as a template for analysis\
\
I produced the spkr_db_amp and rob_db_amp csv files. These I will use in the analysis section.

I created data base for analysis for the robomoth and speaker data. However, I when adding the zeros there's some issues with the effort. apparently theres several site in 2023 for viz04 and viz03 that don't have effort or are not present in the

------------------------------------------------------------------------

6/19/2021\
\
to add zeros to the data for robomoth I had to calculate the effort using the amp files and the dates and times to multiply the duration of each file for the lenght for each recording and thus get the total sampling. I am concern that this might miss the scrubbed files. So I have to double check that when I calculate the amplitude I did it for all the files and not just the files that had not been scrubbed in sonobat.\
\
I also discover that there's a mistake between the names of vizcaine files in the amp files and the robomoth files thus a bunch of files had missing effort. I am working on fixing this issue.\
\
I finally pieced together the to files for spkr and robomoth data.\
spkr_db_v2.csv\
rob_db_v2.csv

Started the analysis and it seems like the best model for robomoth is m7, m11, and m10. I need to decide what will be the best.

------------------------------------------------------------------------

6/24/2025

I have spent this week improving and polishing the models for the spkr and robomoth data. Now we have final model for both

m11 for robomoth

m11_s for spkr

I spend some time discussing the results.

------------------------------------------------------------------------

6/24/2026

I started working with the sm3 data for bats and making the rules for what to keep and what to take.\
Next step is to merge the sm3 data iD from kpro with the sonobat buzz counts.\

------------------------------------------------------------------------

7/7/2026

- I exported the sm3 buzz data to paste to with the bat combined and make a data set to assess if there's more or less calls at lit sites.

- I modified the effort calculation in the prep_for_glmm_v2 script

- I change the species that needed re-coding using a series of rules in the prep_for_glmm_v2 script and these should be ready to go.

- Now:

  - merge effort with bat_combines

  - merge sm3 buzz data with bat combined to get sp level and buzz together

  - analyze both data sets.

- I created the bat_zero_db. It has corrected zeros for when a bat was not recorded but the site was monitored. this should be ready to analyze with the glmm_v4 and this will be updated to v5.

- need to create the sm3 buzz file to merge the ids from the bmzero into the buzz data set.

- I updated the miller data set and this might be use to see if the patters of the glmmv4 hold.

- also created the sm3_buzz_sp but I don't thin it is workng yet. I need to find a way to merge the column sp from the the bat_clean_v3 file to the sm3 buzz.

to do: add the predictors, standardize, run correlation for the bat_zero_db
...
I worked the effort data. Now we have calculations for effort done with the actual SM3 recordings.\
these live in the data_for_analysis\>effort folder.\

------------------------------------------------------------------------

7/15/2026

I ran some model for the sm3 data without the buzzes. I even have some graphs now. However the moon predictor need to be reran with extended dates and I also need to add the miller data.\
other than that it is good for describing.

------------------------------------------------------------------------

7/16/2026

I updated the moon_pred. R script and now all the files have the proper moon predictor.\
I created the moon_pred_v2 to update the moon predictor.\
Now I should be able to run the model in the glmm_v5\
\
...
Before doing any analysis we will filter out calls from parhes and mycal
...
I reran the models and wrote the description of the results in the model.\

------------------------------------------------------------------------

7/28/2026

I need to correct the labels in the sp by jday marginal effect plot for the glmm_v5 script\
- I need to run the analysis for the sm3 data

- correct the analysis for the robomoth and speakrs so I can show which species show actual evidence like in the models for sm3 calls\

------------------------------------------------------------------------

7/29/2026

I almost got the data ready to write for the sm3 data and start the analysis.\

I stil need to correct the results in the sonobat data.\
\
...
Sm3\_ buzz data start analysis.

still need to calculate the uncertainty for the robomoth and speaker data.

I was looking at the data and I see that the there are sites/ night observations with more than 2000 total buzzes for one species.\
these might need to be double check on the raw data

![](images/clipboard-2567568388.png)

I created the script sm3_glmm_v1 and the model m1.8 will be taken as the final model for now.\

------------------------------------------------------------------------

7/31/2026

I was able to describe the results for model m1.8 for feeding buzz activity.\
I still need to fix the speaker and robomoth data.\
I need to organize the plot for the temp, wind, and moon predictors in the model m1.7 for overal activity so they occupy less space. I will use patchwork to make something similar to the predictors for buzz activity see below.

![](images/clipboard-3293302800.png)

next we need to rewrite the section for robomoth and speakers.\
\
...
I am still fixing the robomoth and speaker section but we already started it.\
I migg need to plot the spkr and robomoth additional marginal effects plots. I will fix these sections and then that should be it for the results. I think I can send it for feedback about the results.\
\
...
8/3/2026\
\
I created several tables and graphs for the analysis across speakers sm3 buzz and sm3 activity. all this have complemented the manuscript result section and I feel it is ready for some discussion. However, it might need to be open for feed back before I discuss the results.\
\
...
8/4/2026\
\
Before sending it for feedback, I need to get work on the dbrda analysis and update it with the latest data sets. I am working on the dbrda\
I updated the dbrda analysis and the graph for the manuscript. It is almost ready for feedback.\
\
...
I am updating the bat_delta_sunset.R script and I had to write the bat_clean_v3 from the prep for glmm_v2 script to use as the data analysis table as it is a cleaner and updated version of the bat_combined previously used in the delta script.\
the script works but I feel I need to polish the graph. I might include the overlap result next to each name.

![](images/clipboard-2628932383.png)

------------------------------------------------------------------------

8/27/2026

I am trying to update the analysis for the speaker and robomoth data. Now we need to filter calls according to their amplitude.\
![](images/clipboard-2653759936.png)

In the past I was reading everything to the left of -20 db. I was calling them faint calls. Supervisor suggest to analyze everything to the right of -40 dBfs. In other words we want the calls that are the loudest. Another thing that needs to be modified is some kind of division. We need to get the total calls of bats in a site from the sm3 and then divide the number of bat calls at speakers and robomoths by that day's total. He calls this controlling for abundance.

------------------------------------------------------------------------

8/28/2026

I am controlling for abundance for the robomoth and spkr data. I loaded the sm3 data and I will figure out how to divide them.\

------------------------------------------------------------------------

8/31/2026

I decided to do the offset using the sm3 data as background and offset. I have updated the rob_db and the spkr_db dataset.

------------------------------------------------------------------------

9/01/2026

I modified the models for spekr and robomoth. these are almost ready to build a table of uncertainty and marginal effects plots.\
\
9/02/2026\
\
I kept updating the plots and making sure the model were fit properly. \

![](images/clipboard-3219067985.png)
